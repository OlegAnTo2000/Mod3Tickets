<div class="ticket-comment-row ticket-comment" id="comment-{$id}" data-id="{$id}">
	<div class="ticket-comment-body{$guest ?: ''}">
		<div class="ticket-comment-header">
			...
			<span class="ticket-comment-rating{$can_vote ?: ''}{$cant_vote ?: ''}">
				...
				<span class="vote plus{$voted_plus ?: ''}" title="{$_modx->lexicon('ticket_like')}">
	...
	<a href="{$_modx->makeUrl($section.id, '', '', 'full')}" class="ticket-comment-section">
		<i class="glyphicon glyphicon-folder-open"></i>
		{$section.pagetitle ?: ''}
	</a> &rarr;
	<a href="{$_modx->makeUrl($ticket.id, '', '', 'full')}" class="ticket-comment-ticket">{$ticket.pagetitle ?: ''}</a> &nbsp;
