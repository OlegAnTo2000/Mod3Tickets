    ...
    switch ($options[xPDOTransport::PACKAGE_ACTION]) {
        ...
        case xPDOTransport::ACTION_UPGRADE:
            ...
            if ($policy = $modx->getObject(modAccessPolicy::class, ['name' => 'TicketUserPolicy'])) {
                if ($template = $modx->getObject(modAccessPolicyTemplate::class, ['name' => 'TicketsUserPolicyTemplate'])) {
            ...
            if ($policy = $modx->getObject(modAccessPolicy::class, ['name' => 'TicketSectionPolicy'])) {
                if ($template = $modx->getObject(modAccessPolicyTemplate::class, ['name' => 'TicketsSectionPolicyTemplate'])) {
                    $policy->set('template', $template->get('id'));
                    $policy->save();
                } else {
