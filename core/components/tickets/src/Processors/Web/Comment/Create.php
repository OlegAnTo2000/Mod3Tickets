...
class Create extends CreateProcessor
{
	...
	public function beforeSet()
	{
		...
		if (!$this->thread = $this->modx->getObject(TicketThread::class, array('name' => $tid, 'deleted' => 0, 'closed' => 0))) {
			return $this->modx->lexicon('ticket_err_wrong_thread');
		} elseif ($pid = $this->getProperty('parent')) {
			if (!$parent = $this->modx->getObject(TicketComment::class, array('id' => $pid, 'published' => 1, 'deleted' => 0))) {
				...
				if ($section = $ticket->getOne('Section')) {
					...
					if (isset($ratings['min_comment_create']) && $ratings['min_comment_create'] !== '') {
						if ($profile = $this->modx->getObject(TicketAuthor::class, $this->object->get('createdby'))) {
							...
							if ($rating < $min) return $this->modx->lexicon('ticket_err_rating_comment', array('rating' => $min));
						}
					}
				}
			}
